...
def load_features(feature_name: str, train_df, test_df):
    ...
    optuna_embed_match = re.match(
        r"^chemprop_pretrain_optuna_trial(\d+)_embed$", feature_name
    )
    if optuna_embed_match:
        # 384d MPNN.fingerprint() embeddings from Optuna-tuned chemprop
        # pretrain ckpts (Trial 10/11 from Phase 2 hparam search).
        # Generated by run_chemprop_embed_extract.py with --ckpt
        # checkpoints/chemprop_pretrain_optuna/trial_<NN>/pretrain.pt.
        # SWAP candidate for chemprop_pretrain_embed (same family,
        # tuned hparams). r ~0.92 with default-seed embed.
        trial_num = int(optuna_embed_match.group(1))
        embed_path = REPO_ROOT.joinpath(
            "data", f"chemprop_pretrain_optuna_trial{trial_num}_embed.parquet"
        )
        if not embed_path.exists():
            raise SystemExit(
                f"Missing {embed_path}. Run "
        ...
        X_test = np.nan_to_num(X_test, nan=0.0, posinf=0.0, neginf=0.0)
        print(
            f"  chemprop_pretrain_optuna_trial{trial_num}_embed: {X_train.shape[1]} dims "
            f"(train {X_train.shape[0]} / test {X_test.shape[0]})"
        )
        return X_train, X_test
