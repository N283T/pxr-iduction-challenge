...
pretrain``, replaces the final linear projection (``lin2``) with
``nn.Identity()`` so the forward pass returns the 512d molecular readout
(post-GRU, pre-final projection), then batch-extracts embeddings for
all compounds in the database.
...
def load_model() -> AttentiveFP:
    ...
    # Probe first compound to learn in_channels + edge_dim
    ...
    # The pretrain script saves a dict wrapping the state_dict:
    #   {"state_dict": ..., "params": ..., "target_means": ..., ...}
...
@torch.no_grad()
def extract_embeddings(model: AttentiveFP, graphs: list) -> np.ndarray:
    n = len(graphs)
...
def main() -> None:
    ...
    print(f"  NaN count: {int(np.isnan(emb).sum())}")
