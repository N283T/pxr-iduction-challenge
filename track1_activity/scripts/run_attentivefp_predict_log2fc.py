...
the AttentiveFP pretrain checkpoint.

Phase 1 of issue #115 (log2fc_pred ensembling). Mirrors
run_chemprop_predict_log2fc.py but for the AttentiveFP (PyG) pretrain.
Unlike run_attentivefp_embed_extract.py, does NOT replace the final
lin2 projection; keeps the full forward path to get the 2-head z-scored
log2_fc prediction, then un-z-scores using the pretrain target_means/stds.

Output: data/attentivefp_pretrain_log2fc_predictions.parquet
        index=compound_id, columns=[log2fc_8p25_pred, log2fc_33_pred]

Usage:
    pixi run python track1_activity/scripts/run_attentivefp_predict_log2fc.py
"""
...
CKPT_DIR = REPO_ROOT.joinpath("track1_activity", "checkpoints", "attentivefp_pretrain")
PRETRAIN_PATH = CKPT_DIR.joinpath("pretrain.pt")
META_PATH = CKPT_DIR.joinpath("pretrain_meta.json")
OUT_PATH = REPO_ROOT.joinpath("data", "attentivefp_pretrain_log2fc_predictions.parquet")
...
@torch.no_grad()
def predict_all(model: AttentiveFP, graphs: list) -> np.ndarray:
    n = len(graphs)
