...
def train_and_predict(params, tokenizer, train_smiles, train_targets, val_smiles, val_targets, test_smiles=None):
...
def run_final_cv(best_params, tokenizer, train_smiles, y_train, test_smiles, test_df, outer_splits, split_name):
    """Run full outer CV with best params and save results."""
    best_params["max_epochs"] = 80
    best_params["patience"] = 12
    ...
    for fold, (train_idx, val_idx) in enumerate(outer_splits):
        ...
        val_pred, test_pred = train_and_predict(
            best_params, tokenizer, tr_smi, tr_y, va_smi, va_y, test_smiles
        )
...
def main():
    ...
    if args.split == "umap":
        split_fn = lambda n, s: umap_split_indices(train_smiles, n_splits=n, n_clusters=args.n_clusters, seed=s)
    else:
    ...
    oof_metrics = run_final_cv(
        study.best_params, tokenizer, train_smiles, y_train,
        test_smiles_list, test_df, outer_splits, args.split,
    )
