"""GatedGCN (ResGatedGraphConv) hyperparameter tuning with Optuna.

PyG-based residual gated graph convolutional network with edge features.
ResGatedGraphConv implements a sigmoid-gated neighbor aggregation with an
edge-conditioned gate — conceptually close to the "GatedGCN" architecture
from Bresson & Laurent (2017). Uses UMAP split CV and saves OOF
...
class GatedGCNModel(nn.Module):
    """Stack of ResGatedGraphConv layers with edge conditioning."""
...
def main():
    parser = argparse.ArgumentParser(description="GatedGCN Optuna tuning (PyG)")
    ...
    name = f"gatedgcn_optuna_{args.split}"
    print(f"\n{'=' * 60}")
    ...
    exp_id = record_experiment(
        name=name,
        description=f"GatedGCN (ResGatedGraphConv) Optuna-tuned ({args.split} split)",
        model_type="gatedgcn",
