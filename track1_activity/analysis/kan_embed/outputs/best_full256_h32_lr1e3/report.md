...
Run name: `best_full256_h32_lr1e3`
Experiment: `kan_chemprop_pretrain_embed_best_full256_h32_lr1e3_umap`
...
KAN passes the loose single-model KAN gate, but the follow-up Caruana ADD diagnostic below rejects it as a production-pool member for now.

## Caruana ADD Diagnostic

One-off ADD bakeoff against the current 9-member `ENSEMBLE_MODELS` gave this KAN member only `0.0015` Caruana weight. The extended-pool OOF improvement was again driven mostly by reallocation among existing top members, not by meaningful KAN mass.

OOF Pearson correlations vs key members:

- `tabpfn_chemprop_pretrain_embed_umap_default`: `0.9637`
- `tabpfn_cheme_2d_full_boltz_log2fc_pred_optuna_trial10_seed5ens_umap_default`: `0.9348`
- `tabpfn_gatedgcn_pretrain_embed_umap_default`: `0.9176`

Interpretation: KAN works as a real regressor on full frozen embeddings, but it is not a new ensemble axis relative to the existing ChemProp/TabPFN family.
